...
exports.adicionaReactivePower = async (ultimoDadoMQTT) => {
  ...
  try {
    ...
    const { qa, qb, qc, qt } = ultimoDadoMQTT;
    if ([qa, qb, qc, qt].some((val) => val === undefined || val === null)) {
      console.error("Erro: algum valor de potência ativa está indefinido.");
      return;
    }

    const query = `
      INSERT INTO REACTIVEPOWER (ID, TIME, PHASEA, PHASEB, PHASEC, PHASET)
      VALUES ('${novoId}', '${dataFormatada}', '${qa}', '${qb}', '${qc}', '${qt}')
    `;
    ...
    console.error("Erro ao inserir REACTIVEPOWER:", error.message);
    if (error.response) {
