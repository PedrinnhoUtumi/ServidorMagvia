...
exports.adicionaApparentPower = async (ultimoDadoMQTT) => {
  ...
  try {
    ...
    const { sa, sb, sc, st } = ultimoDadoMQTT;
    if ([sa, sb, sc, st].some((val) => val === undefined || val === null)) {
      console.error("Erro: algum valor de potência ativa está indefinido.");
      return;
    }

    const query = `
      INSERT INTO APPARENTPOWER (ID, TIME, PHASEA, PHASEB, PHASEC, PHASET)
      VALUES ('${novoId}', '${dataFormatada}', '${sa}', '${sb}', '${sc}', '${st}')
    `;
    ...
    console.error("Erro ao inserir APPARENTPOWER:", error.message);
    if (error.response) {
