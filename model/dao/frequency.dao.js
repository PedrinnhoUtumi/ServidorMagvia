...
exports.adicionaFrequency = async (ultimoDadoMQTT) => {
  ...
  try {
    ...
    const { freq } = ultimoDadoMQTT;
    if ([freq].some((val) => val === undefined || val === null)) {
      console.error("Erro: algum valor de potência ativa está indefinido.");
      return;
    }

    const query = `
      INSERT INTO FREQUENCY (ID, TIME, FREQ)
      VALUES ('${novoId}', '${dataFormatada}', '${freq}')
    `;
    ...
    console.error("Erro ao inserir FREQUENCY:", error.message);
    if (error.response) {
