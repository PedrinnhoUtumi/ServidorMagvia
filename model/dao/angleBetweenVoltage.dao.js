...
exports.adicionaAngleBetweenVoltage = async (ultimoDadoMQTT) => {
  ...
  try {
    ...
    const { yuaub, yuauc, yubuc } = ultimoDadoMQTT;
    if ([yuaub, yuauc, yubuc].some((val) => val === undefined || val === null)) {
      console.error("Erro: algum valor de potência ativa está indefinido.");
      return;
    }

    const query = `
      INSERT INTO ANGLEBETWEENVOLTAGES (ID, TIME, YUAUB, YUAUC, YUBUC)
      VALUES ('${novoId}', '${dataFormatada}', '${yuaub}', '${yuauc}', '${yubuc}')
    `;
    ...
    console.error("Erro ao inserir ANGLEBETWEENVOLTAGES:", error.message);
    if (error.response) {
