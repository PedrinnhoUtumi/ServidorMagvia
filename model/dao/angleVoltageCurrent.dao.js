...
exports.adicionaAngleVoltageCurrent = async (ultimoDadoMQTT) => {
  ...
  try {
    ...
    const { pga, pgb, pgc } = ultimoDadoMQTT;
    if ([pga, pgb, pgc].some((val) => val === undefined || val === null)) {
      console.error("Erro: algum valor de potência ativa está indefinido.");
      return;
    }

    const query = `
      INSERT INTO ANGLEVOLTAGECURRENT (ID, TIME, PHASEA, PHASEB, PHASEC)
      VALUES ('${novoId}', '${dataFormatada}', '${pga}', '${pgb}', '${pgc}')
    `;
    ...
    console.error("Erro ao inserir ANGLEVOLTAGECURRENT:", error.message);
    if (error.response) {
